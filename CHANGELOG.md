...
- Brave Search engine (`--engine brave`): HTML scraper with CAPTCHA detection, UA rotation, exponential backoff
...
- Auto mode now queries 5 engines in parallel: DuckDuckGo, Google, Bing, Brave, SearXNG
- `repl` and `csv` feature flags: minimal default install, opt-in for REPL/CSV
- 22 new unit tests for Google, Bing, Brave engines

### Changed

- Default install reduced from ~196 to ~151 crates (feature flags)
- `cargo install seekit -F repl,csv` for full installation
